...
const fetchRankings = async () => {
    try {
        const response = await fetch('/series-rankings')
...
<template>
    <div class="min-h-screen flex flex-col items-center justify-center bg-slate-200">
        ...
        <div class="container mx-auto p-6 flex flex-col flex-grow">
            <h1 class="text-2xl font-bold mb-4 text-maincolor">My Rating TV Shows</h1>
            <div v-if="showsByYear.length > 0" v-for="(yearGroup) in showsByYear" :key="yearGroup.year" class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                    <div v-for="ranking in yearGroup.series" :key="ranking.id" 
            ...
            <div class="flex flex-grow items-center justify-center" v-else>
                <h1 class="text-2xl font-bold text-maincolor">There are no series yet.</h1>
            </div>
        </div>
    </div>
</template>
